...
/////end of "get beamId in to list"
    ...
    //pt.setupCanvas(nCh,1, ip);
    pt.setupCanvas(nCh/2,2, ip, 800, 400);
    //pt.setup2D(nCh,1,72,72,"hist2d");
    pt.setup2D(nCh/2,2,72,72,"hist2d"); 
  ...
  change_pd1: for(int j=0;j<idList.size();j++)
  {
    if(fileId <= idList[j])
    {
		...
		for(int i=0;i<pd1.nFrame();i++)
		{
			...
			//sprintf(str, "ch[%d] xbeam_%d.pd1 frame %d",fileId,i);
			...
			if(k<nCh/2){
				plot2DNum=k+nCh/2;
				pt.loadData2Hist2D(pd1.frameDat+k*72*72,plot2DNum);
			}else{
				plot2DNum=nCh-k-1;
				pt.loadData2Hist2D(pd1.frameDat+k*72*72,plot2DNum);	
				}
				sprintf(str, "ch[%d] xbeam_%d.pd1 frame %d",k,fileId,i);
				pt.h2[plot2DNum]->SetTitle(str);
			}
			//cout<<"xxxxxxxxx"<<endl;
